...
`에라토스테네스의 체`에 대해 간략히 설명하자면, 기본적으로 2는 소수로 처리해놓고 [2..Range)를 순회하며

소수인 것들의 배수들에 대해 not prime으로 만들며 반복하는 알고리즘이다.
...
그 예로 $30=2*3*5$ 일 때 $i=2$일 때 $2*15$에서 $30$을 not prime 처리하고, $i=3$일 때 $30$을 중복으로 not prime 처리하게 된다.
...
## 어떻게 줄여야 할까?
`에라토스테네스의 체`에서 중복이 나오는 경우를 잘 살펴보면 소수의 배수들에 not prime 처리를 할 때 `소수 * 합성수`를 했을 때 중복된다는 것을 알 수 있다.
...
## 그럼 spf는 어떻게 구해야 할까?
$x = number * spf(x)$인 x들에 대해 not prime을 처리한다는 것은 말은 쉽지만 쉽게 방법이 생각나지 않는다.

`에라토스테네스의 체`와 비슷한 로직으로 생각하면 편하다.

대략적인 알고리즘은 다음과 같다.
> 일단 수들을 `에라토스테네스의 체`와 비슷하게 [2..RANGE)를 순회하며 그들의 spf배들에 대해 not prime을 칠한다.
> 
> 현재 not prime이 칠해지지 않은 수들은 소수임이 자명하다.
>
...
> $(2 \leq n \leq 3 \centerdot 10^5)$개의 숫자들이 주어지는데, 이 중 몇 개를 지워야 원래 숫자들의 $gcd$보다 남은 숫자들의 > $gcd$가 커질까?
> $(1 \leq {a_i} \leq 1.5 \centerdot 10^7)$
