...
이것을 구해놓으면 각 트럭마다 $c_i \cdot dp[s_i][e_i][r_i+1]$이 필요한 최소 탱크 수이므로 V는 이것들의 $max$값을 취하면 쉽게 구할 수 있을 것이다.
...
튜토리얼을 보니 이것 외에도 다른 방법이 있는 것 같았는데 설명이 제대로 나와있지 않아 그 방법에 대해서는 모르겠다. (아마 `Binary Search`를 사용한 방법인 것 )
